...
func main() {
  ...
  go import_data_csv(csvData1)
  go import_data_csv(csvData2)
  import_data_csv(csvData3)
...
func import_data_csv(data [][]string) {
  // Trong chuong trinh co import "database/sql", trong package "sql" co dinh nghia type DB struct, vay trong chuong trinh minh muon goi struct DB nay thi phai lam nhu the nao?
  // Trong "encoding/csv" co tham sao nao de cau hinh khi doc file csv khong doc dong header ko?
  dbinfo := fmt.Sprintf("user=%s host=localhost password=%s dbname=%s sslmode=disable", DB_USER, DB_PASSWORD, DB_NAME)
